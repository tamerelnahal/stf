...
module.exports.displayName = function(user) {
	  return user.name || user.displayName || user.mail || user.email || user.userPrincipalName
}

module.exports.memberOfList = function(user) {
	var member = []
	user.memberOf.forEach(function(val) {
		var item = val.split(',OU=')
		var group = item[0].split('CN=')
		member.push(group[1])
	})
	return member
}
